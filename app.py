...
def limpiar_comentarios(texto):
    texto = str(texto)
    # Si el comentario empieza por +, -, = o @, le ponemos una comilla para que no se rompa Google Sheets
    if texto.startswith(("=", "+", "-", "@")):
        return "'" + texto
    return texto
...
# --- PANEL DEL ATLETA ---
    ...
    with st.form("formulario_marcas", clear_on_submit=True):
        ...
        if btn_guardar:
            if marca > 0:
                ...
                # APLICAMOS EL ESCUDO ANTES DE GUARDAR
        ...
        if not df_usuario.empty:
            ...
            if not df_editado.equals(df_usuario):
                ...
                if st.button("💾 Guardar Cambios en la Base de Datos"):
                    ...
                    # APLICAMOS EL ESCUDO ANTES DE GUARDAR EDICIONES
    ...
    if not df.empty and not df_usuario.empty:
        ...
        if not df_grafico.empty:
            ...
            # Ocultamos posibles celdas que solo tengan la comilla aislada
